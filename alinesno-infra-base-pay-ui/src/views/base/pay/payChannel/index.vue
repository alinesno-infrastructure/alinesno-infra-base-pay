               ...
               <el-table-column label="操作" align="center" width="100" class-name="small-padding fixed-width">
                  <template #default="scope">
                     <el-tooltip content="配置" placement="top" v-if="scope.row.id !== 1">
                        <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['system:PayChannel:edit']"></el-button>
                     </el-tooltip>
                  </template>
...
function getList() {
   loading.value = true;
   listPayChannel(proxy.addDateRange(queryParams.value, dateRange.value)).then(res => {
      loading.value = false;
      PayChannelList.value = res.rows;
...
function handleDelete(row) {
   const PayChannelIds = row.id || ids.value;
   proxy.$modal.confirm('是否确认删除应用编号为"' + PayChannelIds + '"的数据项？').then(function () {
      return delPayChannel(PayChannelIds);
   }).then(() => {
      getList();
